    ...
    <div class="card card-custom gutter-b card-sticky" data-card="true" id="kt_page_sticky_card">
      ...
      <div class="card-body" style="">
        ...
        <form method="POST" id="form-exilednoname" action="{{ URL::current() }}/../../{{ $data->id }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
          ...
          <div class="form-group row">
            <label class="col-lg-3 col-form-label"> {{ __('system.label.status') }} </label>
            <div class="col-lg-9">
              {{ Form::select('status', ['1' => __('system.label.success'), '2' => __('system.label.pending')], (isset($data->status) ? $data->status : '1'), ['placeholder' => '- ' . __('system.label.select-status') . ' -', 'class' => $errors->has('status') ? 'form-control is-invalid' : 'form-control', 'required' => 'required']) }}
